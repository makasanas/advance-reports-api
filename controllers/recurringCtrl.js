...
module.exports.create = async (req, res) => {
  ...
  try {
    if (req.body.recurring_application_charge) {
      if (
        Object.keys(req.body.recurring_application_charge).indexOf('name') ==
          -1 ||
        Object.keys(req.body.recurring_application_charge).indexOf('price') ==
          -1 ||
        Object.keys(req.body.recurring_application_charge).indexOf(
          'return_url'
        ) == -1 ||
        Object.keys(req.body.recurring_application_charge).indexOf(
          'trial_days'
    ...
    let shopifyResponse = await handleshopifyRequest(
      'post',
      'https://' +
        decoded.shopUrl +
...
module.exports.getPlan = async (req, res) => {
  ...
  const { decoded } = req;
  try {
    rcResponse.data = await commonModel.findOne('activePlan', {
      userId: decoded.id,
    });
  } catch (err) {
    handleError(err, rcResponse);
...
module.exports.activePlan = async (req, res) => {
  ...
  try {
    let plan = await commonModel.findOne('activePlan', {
      userId: decoded.id,
      chargeInfo: { $elemMatch: { id: params.planId } },
    });
    if (!plan) {
      let shopifyResponse = await handleshopifyRequest(
        'post',
      ...
      let updatedPlan = await commonModel.findOneAndUpdate(
        'activePlan',
        { userId: decoded.id },
        data
      );
      ...
      rcResponse.data = await commonModel.findOne(
        'user',
        { _id: decoded.id },
        { accessToken: 0 }
      );
