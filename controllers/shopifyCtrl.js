...
generatorAcessToekn = async req => {
  ...
  try {
    if (securityCheck(req)) {
      ...
      const response = await handleshopifyRequest(
        'post',
...
createShop = async (shop, productCount, shopData) => {
  ...
  try {
    let trial_days;
    let deletedUser = await commonModel.findOne('deletedUser', {
    ...
    let user = await commonModel.findOneAndUpdate(
      'user',
      { shopUrl: shop.myshopify_domain },
      data
    );
    ...
    let plan = {
      ...
      nextMonthStartDate: new Date(
        new Date(utc).getTime() + 30 * 24 * 60 * 60 * 1000
      ),
      chargeInfo: {
    ...
    plan = await commonModel.findOneAndUpdate(
      'activePlan',
      { shopUrl: shop.myshopify_domain },
      plan
    );
  ...
  try {
    ...
    if (!user) {
      ...
      promise.push(
        handleshopifyRequest(
          'get',
          'https://' + shopData.shopUrl + process.env.apiVersion + 'shop.json',
  ...
  try {
    ...
    if (user) {
      ...
      if (user.trial_days && user.trial_start) {
        user.trial_days =
          user.trial_days - date_diff_indays(user.trial_start, Date.now()) < 0
            ? 0
            : user.trial_days - date_diff_indays(user.trial_start, Date.now());
      }

      delete user._id;

      let deleteId = await commonModel.findOneAndUpdate(
        'deletedUser',
        { shopUrl: user.shopUrl },
        { $set: user }
      );
  ...
  try {
    ...
    let webhooks = [
      ...
      },
      {
        webhook: {
          topic: 'orders/create',
          address: hostname + '/webhooks/app/order',
          format: 'json',
        },
      },
      {
    ...
    await Promise.all(promise)
      ...
      .catch(function(err) {
        if (
          err.statusCode == 422 &&
          err.message ==
            '422 - {"errors":{"address":["for this topic has already been taken"]}}'
        ) {
          return true;
        } else {
          throw err;
...
module.exports.getEnabledApp = async (req, res) => {
  ...
  try {
    const AppStatus = await commonModel.findOne('user', {
      shopUrl: req.query.shopUrl,
    });
...
addScript = async decoded => {
  try {
    let scripts = await handleshopifyRequest(
      'get',
      'https://' +
        decoded.shopUrl +
        process.env.apiVersion +
...
removeScript = async decoded => {
  try {
    ...
    await Promise.all(scriptDeleteArray)
